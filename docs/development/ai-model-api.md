...
- `clarification` 仅转换为协作对话中的 AI 消息，不保存或展示在任务详情中。加载历史数据时会把旧记录残留的问题迁移到协作对话，再从任务数据中清除。
...
2026-09-21 验证确认问题路由：模型返回 `clarification` 时，问题只出现在顶部协作对话；对应任务详情不显示“需要确认”或问题正文，浏览器控制台无错误。
